...
def polar2cartesian(outcoords, inputshape, origin, fieldscale=1.0):
...
def exec_polar_expansions(M, Only_Syst=False, with_Syst=True):
    ...
    fieldscale = (
        M.fieldscale
    )  # shrink radial field of view of polar maps by this factor
    ...
    if (abs(dra_off) > 0.0) or (abs(ddec_off) > 0.0):
        RA = RA + ((dra_off / 3600.0) / np.cos(DEC * np.pi / 180.0))
        DEC = DEC + (ddec_off / 3600.0)
        if M.Verbose:
        ...
        if M.DumpAllFitsFiles:
            ...
            pf.writeto(fileout_centered, resamp, hdr2, overwrite=True)
            if M.Hdu_err is not None:
                fileout_centered_err = re.sub(
                    "fullim.fits", "centered_errmap.fits", filename_fullim
                )
                pf.writeto(fileout_centered_err, resamp_errmap, hdr2, overwrite=True)
    ...
    if M.Hdu_err is not None:
        im1rot_err = ndimage.rotate(resamp_errmap, rotangle, reshape=False, order=1)
    ...
    if M.DumpAllFitsFiles:
        ...
        if M.Hdu_err is not None:
            fileout_rotated_err = re.sub(
                "fullim.fits", "rotated_err.fits", filename_fullim
            )
    ...
    if M.DumpAllFitsFiles:
        if M.UVplane:
            fileout_stretched = re.sub("fullim.fits", "squeezed.fits", filename_fullim)
        else:
            fileout_stretched = re.sub("fullim.fits", "stretched.fits", filename_fullim)

        pf.writeto(fileout_stretched, im3, hdr2, overwrite=True)
        if M.Hdu_err is not None:
            fileout_stretched = re.sub(
                "fullim.fits", "stretched_err.fits", filename_fullim
            )
            pf.writeto(fileout_stretched, im3, hdr2, overwrite=True)
    ...
    if M.DumpAllFitsFiles:
        ...
        if M.Hdu_err is not None:
            fileout_polar_err = re.sub("fullim.fits", "polar_err.fits", filename_fullim)
    ...
    if DoRadialProfile:
        if M.Hdu_err is None:
            ...
            im_polar_err[(im_polar_err < 0.5 * M.noise_floor)] = 0.0
            im_polar_weights = 1 / im_polar_err**2
            max_weights = 1 / M.noise_floor**2
            im_polar_weights[(im_polar_weights > max_weights)] = 0.0
            norm_weights = np.sum(im_polar_weights, axis=1)
            Iprof = np.sum(im_polar * im_polar_weights, axis=1) / norm_weights
            Iprof[norm_weights <= 0] = 0.0

            varIprof = (
                np.sum(im_polar**2 * im_polar_weights, axis=1) / norm_weights
                - Iprof**2
            )
        ...
        Nind = 2.0 * np.pi * rrs * np.fabs(cosi) / (bmaj * 3600.0)  # cosi *
        ...
        if wBaseNoise:
            ...
            if M.Verbose:
                print(
                    (
                        ">using noise floor of ",
                        noise_basal,
                        "  from dispersion at radius ",
                        noise_radius,
                    )
                )
        else:
        ...
        if M.DumpAllFitsFiles:
            ...
            fileout_radialprofile = re.sub(
                "fullim.fits", "radial_profile.dat", filename_fullim
            )
        ...
        if M.DumpAllFitsFiles:
            fileout_stretched_av = re.sub(
                "fullim.fits", "stretched_av.fits", filename_fullim
            )
        ...
        if M.DumpAllFitsFiles:
            fileout_drotated = re.sub(
                "fullim.fits", "azim_av_drot.fits", filename_fullim
            )
    ...
    if (DoAzimuthalProfile) or (ProfileExtractRadius > 0):
        ...
        if ProfileExtractRadius > 0:
            ...
            iringrad = np.argmin(np.abs(rs - ringrad))
        elif WholeRing:
        ...
        rmsring2 = np.sqrt(
            np.sum((Rmax_profile - avring) ** 2 * Imax_profile / np.sum(Imax_profile))
        )
    ...
    if XCheckInv:
        im_x = sp.ndimage.geometric_transform(
            im_polar,
            polar2cartesian,
            order=0,
            output_shape=(im_polar.shape[0], im_polar.shape[1]),
    ...
    if PlotAzimuthalProfile:
        ...
        (phis, Rmax_profile) = np.loadtxt(workdir + "rmax_profile.dat", unpack=True)
        ...
        fileout_fig = re.sub("fullim.fits", "fig_azprofile.pdf", filename_fullim)
    ...
    if PlotRadialProfile:
        ...
        plt.setp(axprofile.get_yticklabels(), visible=True)  # , fontsize=6)
        if a_max_plot > 0.0:
            plt.xlim(0.0, a_max_plot)

        # plt.ylim(np.min(Iprof),1.1*np.max(Iprof))
        plt.ylim(-0.1 * np.max(Iprof), 1.1 * np.max(Iprof))

        plt.plot(rrs, rrs * 0.0, color="black", linewidth=0.1, linestyle="solid")
